...
#pragma mark - Shape Loading
                ...
                void parseEllipseShape(const XmlTree &shapeNode, Shape2d &shape) {
                    ...
                    if (rx > 0 && ry > 0) {
                        ...
                        command << "M " << (cx + rx) << "," << (cy);
                        command << " A " << rx << "," << ry << " " << 0 << " " << 0 << " " << 1 << " " << (cx) << ","
                                << (cy + ry);
                        command << " A " << rx << "," << ry << " " << 0 << " " << 0 << " " << 1 << " " << (cx - rx)
                                << "," << (cy);
                        command << " A " << rx << "," << ry << " " << 0 << " " << 0 << " " << 1 << " " << (cx) << ","
                                << (cy - ry);
                        command << " A " << rx << "," << ry << " " << 0 << " " << 0 << " " << 1 << " " << (cx + rx)
            ...
            void parseShape(const XmlTree &shapeNode, ci::Shape2d &shape) {
                ...
                if (name == "path") {
                    parsePathShape(shapeNode, shape);
                    return;
                }
                if (name == "rect") {
                    parseRectShape(shapeNode, shape);
                    return;
                }
                if (name == "circle") {
                    parseCircleShape(shapeNode, shape);
                    return;
                }
                if (name == "ellipse") {
                    parseEllipseShape(shapeNode, shape);
                    return;
                }
                if (name == "line") {
                    parseLineShape(shapeNode, shape);
                    return;
                }
                if (name == "polyline") {
                    parsePolylineShape(shapeNode, shape);
                ...
                if (name == "polygon") {
                    parsePolygonShape(shapeNode, shape);
                    return;
                }
